 ...
 */
test(`Authenticate once then send ${apiCallCount} Warranty Page API requests simultaneously at once without opening tabs`, async ({ browser }, testInfo) => {
    test.setTimeout(0);
    ...
    console.log(`Step 2: Firing ${apiCallCount} direct Warranty Page API requests simultaneously via Promise.all()...`);

    const globalDispatchStart = Date.now();

    const apiPromises = Array.from({ length: apiCallCount }).map(async (_, idx) => {
        const reqId = idx + 1;
        const sectionName = warrantySubSections[idx % warrantySubSections.length];
    ...
    const reportContent = `==================================================
       WARRANTY PAGES AFTER-LOGIN API SCORECARD   
==================================================
Target Module:            Warranty Pages (${warrantySubSections.length} Sections)
...
==================================================`;
    ...
    testInfo.attachments.push({
        name: 'Warranty Pages API Performance Report.txt',
        contentType: 'text/plain',
        body: Buffer.from(reportContent, 'utf-8'),
    });

    testInfo.attachments.push({
        name: 'Warranty Pages API Metrics.json',
    ...
    testInfo.annotations.push({
        ...
        description: `Warranty APIs | Hits: ${results.length} | Passed: ${passedCount} | Duration: ${(totalExecutionTimeMs / 1000).toFixed(1)}s`,
    });

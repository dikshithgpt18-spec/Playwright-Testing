...
test(`Parallel ${tabCount} tabs hitting the EXACT SAME Automation page API simultaneously at once via Promise.all`, async ({ browser }, testInfo) => {
    ...
    // -------------------------------------------------------------
    // Step 3: HIT THE EXACT SAME AUTOMATION PAGE API SIMULTANEOUSLY ACROSS ALL TABS VIA Promise.all()
    // -------------------------------------------------------------
    for (let cycle = 1; cycle <= repeatCount; cycle++) {
        ...
        for (let i = 0; i < automationSubSections.length; i++) {
            ...
            console.log(`\n[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${automationSubSections.length}: ${sectionName}] Firing SAME API hit across all ${tabs.length} tabs simultaneously via Promise.all()...`);
            ...
            const hitResults = await Promise.all(tabs.map(async ({ page, tabIndex }) => {
                try {
                    ...
                    console.log(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${automationSubSections.length}] [Tab ${tabIndex}] SAME API TRIGGERED -> ${sectionName} | Status: ${status} | API Response Time: ${apiResponseTimeMs}ms`);
                    return { tabIndex, sectionName, success: true, apiResponseTimeMs, status };
                } catch (err) {
                    console.warn(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${automationSubSections.length}] [Tab ${tabIndex}] API TRIGGER FAILED -> ${sectionName}:`, err);
            ...
            console.log(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${automationSubSections.length}: ${sectionName}] Completed: ${successfulHits}/${tabs.length} tabs hit SAME API simultaneously via Promise.all().`);
        }

        console.log(`\n[Cycle ${cycle}/${repeatCount}] Completed all ${automationSubSections.length} Automation page SAME API hits across all ${tabs.length} tabs.`);
    }

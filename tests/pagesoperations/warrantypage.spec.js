...
// All sub-sections under Warranty in one single combined array
const warrantySubSections = [
    'Common Data View',
    'Part Return Analysis',
    'Master Data',
    'Part Return Conclusion',
    'Lookup Table'
];
...
test(`Parallel ${tabCount} tabs hitting the EXACT SAME Warranty page API simultaneously at once via Promise.all`, async ({ browser }, testInfo) => {
    ...
    // -------------------------------------------------------------
    // Step 2: Open Tabs safely in batches of 10 to prevent memory issues using Promise.all()
    // -------------------------------------------------------------
    ...
    // -------------------------------------------------------------
    // Step 3: HIT THE EXACT SAME WARRANTY PAGE API SIMULTANEOUSLY ACROSS ALL TABS VIA Promise.all()
    // -------------------------------------------------------------
    for (let cycle = 1; cycle <= repeatCount; cycle++) {
        ...
        for (let i = 0; i < warrantySubSections.length; i++) {
            ...
            console.log(`\n[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${warrantySubSections.length}: ${sectionName}] Firing SAME API hit across all ${tabs.length} tabs simultaneously via Promise.all()...`);

            // Phase A: Ensure Warranty dropdown menu is open across all tabs simultaneously
            await Promise.all(tabs.map(async ({ page }) => {
                const warrantyMenu = page.getByText('Warranty', { exact: true }).or(page.getByText(/Warranty/i)).first();
                ...
                if (!(await itemLocator.isVisible({ timeout: 200 }).catch(() => false))) {
                    await warrantyMenu.click({ force: true }).catch(() => { });
                }
            }));
            ...
            const hitResults = await Promise.all(tabs.map(async ({ page, tabIndex }) => {
                try {
                    ...
                    console.log(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${warrantySubSections.length}] [Tab ${tabIndex}] SAME API TRIGGERED -> ${sectionName} | Status: ${status} | API Response Time: ${apiResponseTimeMs}ms`);
                    return { tabIndex, sectionName, success: true, apiResponseTimeMs, status };
                } catch (err) {
                    console.warn(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${warrantySubSections.length}] [Tab ${tabIndex}] API TRIGGER FAILED -> ${sectionName}:`, err);
            ...
            console.log(`[Cycle ${cycle}/${repeatCount}] [Order ${sectionOrder}/${warrantySubSections.length}: ${sectionName}] Completed: ${successfulHits}/${tabs.length} tabs hit SAME API simultaneously via Promise.all().`);
        }

        console.log(`\n[Cycle ${cycle}/${repeatCount}] Completed all ${warrantySubSections.length} Warranty page SAME API hits across all ${tabs.length} tabs.`);
    }

    const testExecutionTimeMs = Date.now() - testStartedAt;
    const totalHitsFired = tabs.length * warrantySubSections.length * repeatCount;
...
Sub-Sections Tested:            ${warrantySubSections.join(', ')}
...
================================================================================`;
    ...
    testInfo.attachments.push({
        name: 'Warranty Pages Metrics.json',
        contentType: 'application/json',
        body: Buffer.from(
            JSON.stringify(
                {
                    module: 'Warranty Pages',
                    subSectionsCount: warrantySubSections.length,
                    subSections: warrantySubSections,
    ...
    testInfo.annotations.push({
        ...
        description: `Module: Warranty | Tabs: ${tabs.length} | Cycles: ${repeatCount} | Total Hits: ${totalHitsFired} | Duration: ${(testExecutionTimeMs / 1000).toFixed(1)}s`,
    });
